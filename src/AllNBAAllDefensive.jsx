 ...
 * Score = teamWinPct*2.5 + (ppg/max)*3.5 + (rpg/max)*1.2 + (apg/max)*1.6 + fg% + 3p% - tov + (spg/max)*1 + (bpg/max)*0.6 (steals weighted more than blocks).
...
export function buildAllNBATeams(players, teamWinPct, mvpVotes = null) {
  ...
  const score = (r) => {
    ...
    return teamPct * 2.5 + ppgN * 3.5 + rpgN * 1.2 + apgN * 1.6 + fgN + tpN - tovPen + spgN * 1 + bpgN * 0.6;
  };
